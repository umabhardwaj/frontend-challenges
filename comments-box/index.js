...
const createComment = (name, text, settings) => {
  ...
  const buttons = [];
  buttons.push(
    createElement("button", { textContent: "Reply", className: "reply" })
  );
  if (!settings?.hasNoEdit)
    buttons.push(
      createElement("button", { textContent: "Edit", className: "edit" })
    );
  if (!settings?.hasNoDelete)
    buttons.push(
      createElement("button", { textContent: "Delete", className: "delete" })
    );
